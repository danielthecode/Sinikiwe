...
function UploadPortraiture() {
  ...
  function UploadImage () {
    ...
    let uploadPromises = images.map(image => {
      ...
      const imageRef = ref(storage, `Portraiture/${newImage.name + v4()}`)
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
           ImageService.addImage(collection(db, "portraiture"),downloadURL)
...
const handleLogout = (e) => {
  ...
  return (
    <div className="upload-container">
      <div className="title">Upload Portraiture Image <button onClick={handleLogout}>Logout</button></div>
        ...
        {({
          ...
          <div className="upload__image-wrapper">
            <div className="controls">
              <div className="select-button" style={isDragging ? { color: 'red' } : undefined}
            ...
            <div className="upload-button" onClick={UploadImage}>
                <div className="icon"><AiOutlineSend/></div>
                Upload-Images
              </div>
            </div>
            ...
            <div className="progress">
                <CircularProgressbar value={progressP} text={`${progressP}`} styles={buildStyles({textSize: "12px"})} />
      </div>
          </div>
        )}
      </ImageUploading>
...
export default UploadPortraiture